/*
	***	***	***	***	***	***	***	***	***	***	***	***	***	***	***	***	***	***	***	***	***	***	***	***	***
	***		Raspberry Pi gpio hardware interrupt counter.												***
	***		This is the user level code.  The ISR is in pulsecounter.c									***
	***																								***
	***																								***
	***		by Tamás Gere , February 2019.       GPL licensed										***
...
int main(int argc, const char * argv[])
{
	...
	if(argc!=2){
		fprintf(stderr,"Usage :pulsecounter  </path/file> %d\n",argc);
		return -1;
	}
...
static irq_user_info  *getDriver_myMap_info ( void)
{
	...
	configfd = open("/sys/kernel/debug/pulsecounter_mmap", O_RDWR);
	if(configfd < 0) {
		perror("open");
		return 0;
	}
